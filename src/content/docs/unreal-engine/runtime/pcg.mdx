...
This section provides overview and insights of the procedural content generation (PCG) framework of Unreal Engine. More resources:

- a [video tutorial series](https://dev.epicgames.com/community/learning/tutorials/1wro/unreal-engine-pcg-tutorial-series)
- official PCG graph [node reference](https://dev.epicgames.com/documentation/unreal-engine/procedural-content-generation-framework-node-reference-in-unreal-engine)
...
### Blueprint construction and graph updates

If the PCG component is being used inside a parent Actor (and not in the default PCG volume), it is sometimes useful to have control parameters as the parent actor properties. In that case, the PCG component needs to forcibly updated each through the actor's construction script with `Graph->GenerateLocal()`